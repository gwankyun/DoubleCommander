...
if bAck then
    if string.match(sAnswer, "^f") then
        DC.ExecuteCommand("cm_GoToFirstFile")
        cm.refresh()
    elseif string.match(sAnswer, "^l") then
        DC.ExecuteCommand("cm_GoToLastFile")
        cm.refresh()
    elseif string.match(sAnswer, "^F") then
        DC.ExecuteCommand("cm_GoToFirstEntry")
        cm.refresh()
    elseif string.match(sAnswer, "^L") then
        DC.ExecuteCommand("cm_GoToLastEntry")
        cm.refresh()
    elseif string.match(sAnswer, "^s") then
        dc_sort(string.sub(sAnswer, 2))
    end
end
